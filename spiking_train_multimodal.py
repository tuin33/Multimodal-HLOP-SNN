...
from models.spiking_cnn_hlop_multimodal import spiking_cnn
...
def test(args, model, img_x, audio_x, y, task_id):
    ...
    test_samples = 0
    batch_idx = 0
    # print("xtest shape:",img_x.size(0))
    # exit()
    ...
    with torch.no_grad():
        for i in range(0, len(r), args.b):
            ...
            # print(f"out.argmax = {out}")
            # print(f"out.argmax = {out.argmax(1)}")
            # print(f"label = {label}")
...
def main():
    ...
    for k, ncla in taskcla:
        ...
        # task_list.append(k)
        img_xtrain=data[k]['train']['x']
        audio_xtrain=torch.zeros((img_xtrain.size(0), 104))
        ytrain=data[k]['train']['y']
        img_xtest =data[k]['test']['x']

        audio_xtest=torch.zeros((img_xtest.size(0), 104))
        ytest =data[k]['test']['y']
        task_list.append(k)
        ...
        if args.replay:
            ...
            for c in range(ncla):
                ...
                while num > 0:
                    if ytrain[index] == c:
                        replay_data[task_id]['img_x'].append(img_xtrain[index])
                        replay_data[task_id]['audio_x'].append(audio_xtrain[index])
                        replay_data[task_id]['y'].append(ytrain[index])
                        num -= 1
                    index += 1
            replay_data[task_id]['img_x'] = torch.stack(replay_data[task_id]['img_x'], dim=0)
            replay_data[task_id]['audio_x'] = torch.stack(replay_data[task_id]['audio_x'], dim=0)
        ...
        if task_id == 0:
            print(f"ncla = {ncla}")
            model = spiking_MLP_multimodal(multi_fusion_dim, snn_setting, num_classes=ncla, n_hidden=800, ss=args.sign_symmetric, fa=args.feedback_alignment, hlop_with_wfr=hlop_with_wfr, hlop_spiking=args.hlop_spiking, hlop_spiking_scale=args.hlop_spiking_scale, hlop_spiking_timesteps=args.hlop_spiking_timesteps)
            # model = spiking_cnn(multi_fusion_dim, snn_setting, num_classes=ncla, ss=args.sign_symmetric, hlop_with_wfr=hlop_with_wfr, hlop_spiking=args.hlop_spiking, hlop_spiking_scale=args.hlop_spiking_scale, hlop_spiking_timesteps=args.hlop_spiking_timesteps, proj_type=args.hlop_proj_type)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for i in range(0, len(r), args.b):
                ...
                img_x = img_xtrain[index].float().cuda()
                audio_x = audio_xtrain[index].float().cuda()
                # repeat for time steps
                # img_x = img_x.unsqueeze(1)
                # img_x = img_x.repeat(1, args.timesteps, 1, 1, 1)

                # audio_x = audio_x.unsqueeze(1)
                # audio_x = audio_x.repeat(1, args.timesteps, 1)

                label = ytrain[index].cuda()
                optimizer.zero_grad()
                if task_id == 0:
                    if args.baseline:
                        out = model(img_x, audio_x, task_id, projection=False, update_hlop=False)
                    else:
                        if epoch <= args.hlop_start_epochs:
                            out = model(img_x, audio_x, task_id, projection=False, update_hlop=False)
                        else:
                            out = model(img_x, audio_x, task_id, projection=False, update_hlop=True)
                else:
                    if args.baseline:
                        out = model(img_x, audio_x, task_id, projection=False, proj_id_list=[0], update_hlop=False, fix_subspace_id_list=[0])
                    else:
                        if epoch <= args.hlop_start_epochs:
                            out = model(img_x, audio_x, task_id, projection=True, proj_id_list=[0], update_hlop=False, fix_subspace_id_list=[0])
                        else:
                            out = model(img_x, audio_x, task_id, projection=True, proj_id_list=[0], update_hlop=True, fix_subspace_id_list=[0])
                ...
                # measure accuracy and record loss
                prec1, prec5 = accuracy(out.data, label.data, topk=(1, 5))
                losses.update(loss, img_x.size(0))
                top1.update(prec1.item(), img_x.size(0))
                top5.update(prec5.item(), img_x.size(0))
            ...
            test_loss, test_acc = test(args, model, img_xtest, audio_xtest, ytest, task_id)
        ...
        for ii in np.array(task_list)[0:task_id+1]:
            ...
            img_xtest =data[ii]['test']['x']
            audio_xtest=torch.zeros((img_xtest.size(0), 104))
            ytest =data[ii]['test']['y']

            # img_xtest =data[ii]['train']['img']
            # audio_xtest =data[ii]['train']['audio']
            # ytest =data[ii]['train']['labels'] 
            _, acc_matrix[task_id,jj] = test(args, model, img_xtest, audio_xtest, ytest, ii) 
            jj +=1
        print('Accuracies =')
        ...
        if args.replay and task_id >= 1:
            ...
            for epoch in range(1, args.replay_epochs + 1):
                ...
                task_data_num = replay_data[0]['img_x'].size(0)
                r = np.arange(task_data_num)
                np.random.shuffle(r)
                for i in range(0, task_data_num, batch_per_task):
                    optimizer.zero_grad()
                    for replay_taskid in range(task_id+1):
                        img_xtrain = replay_data[replay_taskid]['img_x']
                        audio_xtrain = replay_data[replay_taskid]['audio_x']
                        ...
                        out = model(img_x, audio_x, replay_taskid, projection=False, update_hlop=False)
                        label = label.squeeze(1).long()
                        loss = F.cross_entropy(out, label)
                        loss.backward()
                    optimizer.step()
